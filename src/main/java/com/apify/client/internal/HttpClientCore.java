...
public final class HttpClientCore {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(HttpClientCore.class);
  ...
  public HttpClientCore(HttpClient backend, String token, String userAgent, RetryConfig retry) {
  ...
  public ApiResponse callWithHeaders(
      String method,
      String url,
      byte[] body,
      String contentType,
  ...
  public static final class Compressed {
    ...
    public Compressed(byte[] body, String encoding) {
      this.body = body;
  ...
  private static byte[] brotli(byte[] data) {
    try {
      ...
      // UncheckedIOException, not ApifyTransportException: a local encoding failure never touched
      // the network, so it is not a transport failure.
  ...
  private static byte[] gzip(byte[] data) {
    ...
    try (GZIPOutputStream gz = new GZIPOutputStream(out)) {
      ...
      // practice. UncheckedIOException, not ApifyTransportException: a local encoding failure
      // never touched the network, so it is not a transport failure.
      throw new java.io.UncheckedIOException(e);
    }
  ...
  /**
   * Reports whether a transport failure was caused by a request timeout. Recognizes both the JDK
   * default backend's {@link HttpTimeoutException} and the more general {@link
   * SocketTimeoutException} some custom {@link HttpClient} implementations throw for a timeout (the
   * {@link HttpClient} interface contract only guarantees the cause is an {@code IOException}, not
   * a specific backend's exception type) — a custom backend that throws one of these for a timeout
   * gets the same {@code doNotRetryTimeouts} behavior as the default backend.
   */
  private static boolean isTimeout(ApifyTransportException e) {
  ...
  /**
   * Builds an {@link ApifyApiException} from an API error response body, deserializing the {@code
   * {"error": {...}}} envelope directly into a typed {@link ApiErrorEnvelope} rather than
   * navigating a raw {@code JsonNode} field by field.
   */
  public static ApifyApiException buildApiError(
    ...
    try {
      ApiErrorEnvelope envelope = Json.parse(body, ApiErrorEnvelope.class);
      ...
      // Body was not the expected error envelope (e.g. an HTML error page from a proxy, or
      // malformed/empty JSON). Fall through to the generic message below.
    }
    if (message == null) {
      message =
  ...
  /**
   * Returns the path+query portion of a URL, for error reporting, by parsing it as a {@link URI}
   * rather than with manual string slicing.
   */
  public static String extractPath(String url) {
    try {
      ...
      // Not expected for a URL this client built itself; fall back to an empty path rather than
      // let error reporting itself fail.
      return "";
    }
  }
