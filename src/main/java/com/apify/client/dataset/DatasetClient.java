...
public final class DatasetClient {
  ...
  public DatasetClient(HttpClientCore http, String baseUrl, String resourcePath, String id) {
    this.http = http;
    this.ctx = ResourceContext.single(http, baseUrl, resourcePath, id);
  }

  /**
   * As {@link #DatasetClient(HttpClientCore, String, String, String)}, additionally overriding the
   * origin used for public, shareable URLs. Used by {@code ApifyClient.dataset(String)} for
   * top-level datasets, which support a public-base-URL override; run-nested datasets (via {@link
   * #nested}) never do. Resolving the override here, before the field assignment, keeps {@code ctx}
   * (and so this client) immutable from construction on rather than mutated after the fact.
   */
  public DatasetClient(
      HttpClientCore http, String baseUrl, String resourcePath, String id, String publicBaseUrl) {
    this.http = http;
    ResourceContext base = ResourceContext.single(http, baseUrl, resourcePath, id);
    this.ctx = publicBaseUrl != null ? base.withPublicOrigin(publicBaseUrl) : base;
  }
