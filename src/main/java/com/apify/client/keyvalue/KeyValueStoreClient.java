...
public final class KeyValueStoreClient {
  ...
  public KeyValueStoreClient(HttpClientCore http, String baseUrl, String resourcePath, String id) {
    this.ctx = ResourceContext.single(http, baseUrl, resourcePath, id);
  }

  /**
   * As {@link #KeyValueStoreClient(HttpClientCore, String, String, String)}, additionally
   * overriding the origin used for public, shareable URLs. Used by {@code
   * ApifyClient.keyValueStore(String)} for top-level stores, which support a public-base-URL
   * override; run-nested stores (via {@link #nested}) never do. Resolving the override here, before
   * the field assignment, keeps {@code ctx} (and so this client) immutable from construction on
