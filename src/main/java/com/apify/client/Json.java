...
final class Json {
  ...
  static <T> T parse(byte[] body, JavaType type) {
    try {
  ...
  static <T> T parse(byte[] body, TypeReference<T> type) {
    try {
      return MAPPER.readValue(body, type);
    } catch (IOException e) {
      throw new UncheckedIOException("failed to parse API response", e);
   ...
   */
  static <T> T parseData(byte[] body, JavaType dataType) {
  ...
  static <T> T parseData(byte[] body, Class<T> dataClass) {
